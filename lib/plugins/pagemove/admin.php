    ...
    function handle() {
      ...
      if ( isset($_REQUEST['ns']) or isset($_REQUEST['pagename']) ) {
        ...
        elseif ( @file_exists($opts['new_path']) ) {
          $this->errors[] = $this->lang['pm_existing1'].$opts['newname'].
                           $this->lang['pm_existing2'].($opts['newns'] == '' ? $this->lang['pm_root'] : $opts['newns']);
        }
        ...
        if ( count($this->errors) == 0 ) {
          ...
          saveWikiText($opts['new_id'], $this->text, $this->lang['pm_movedfrom'].$ID);
...
/*        foreach($namesp as $row) {
          if ( auth_quickaclcheck($row['id'].':*') >= AUTH_CREATE or $row['id'] == $opts['ns'] ) {
            ptln ( '        <td><input type="radio" name="ns" value="'.
                    ($row['id'] ? $row['id'] : ':').
                    ($_REQUEST['ns'] ?
                      (($row['id'] ? $row['id'] : ':') == $_REQUEST['ns'] ? '" CHECKED>' : '">') :
                      ($row['id'] == $opts['ns'] ? '" CHECKED>' : '">') ).
...
*/
    ...
    function _pm_updatebacklinks($id, $links, $opts, &$brackets)
    {
      ...
      saveWikiText($id, $text, $this->lang['pm_linkchange1'].$ID.$this->lang['pm_linkchange2'].$opts['new_id']);
      unlock($id);
    }
